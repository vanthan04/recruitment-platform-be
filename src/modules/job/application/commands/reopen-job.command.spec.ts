import {
  ReopenJobCommand,
  ReopenJobHandler,
} from '@/modules/job/application/commands/reopen-job.command';
...
describe('ReopenJobHandler', () => {
  ...
  beforeEach(() => {
    ...
    handler = new ReopenJobHandler(jobRepository);
  });
  ...
  it('throws JobNotFoundException when the job does not exist', async () => {
    ...
    await expect(
      handler.execute(new ReopenJobCommand('recruiter-1', 'job-1')),
  ...
  it('rejects a recruiter who did not post the job', async () => {
    ...
    await expect(
      handler.execute(new ReopenJobCommand('some-other-recruiter', 'job-1')),
    ).rejects.toThrow(UnauthorizedDomainException);
    expect(jobRepository.update).not.toHaveBeenCalled();
  });

  it('throws JobNotClosedException when the job is not currently closed', async () => {
    jobRepository.findById.mockResolvedValue(
      makeJob({ status: JobStatus.OPEN }),
    );

    await expect(
      handler.execute(new ReopenJobCommand('recruiter-1', 'job-1')),
    ).rejects.toThrow(JobNotClosedException);
    expect(jobRepository.update).not.toHaveBeenCalled();
  });

  it('reopens a closed job and returns the updated DTO', async () => {
    const job = makeJob();
    jobRepository.findById.mockResolvedValue(job);
    jobRepository.update.mockImplementation(async (j) => j);

    const result = await handler.execute(
      new ReopenJobCommand('recruiter-1', 'job-1'),
    );

    expect(jobRepository.update).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'job-1', status: JobStatus.OPEN }),
    );
