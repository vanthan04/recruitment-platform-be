...
export class RescheduleInterviewHandler
  implements ICommandHandler<RescheduleInterviewCommand, InterviewResponseDto>
{
  constructor(
  ...
  async execute({ recruiterId, interviewId, input }: RescheduleInterviewCommand): Promise<InterviewResponseDto> {
    ...
    if (job.postedById !== recruiterId) {
      throw new UnauthorizedDomainException('Only the job poster can reschedule this interview');
    }
