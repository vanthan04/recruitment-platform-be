...
} from './mark-interview-no-show.command';
...
describe('MarkInterviewNoShowHandler', () => {
  ...
  let handler: MarkInterviewNoShowHandler;
  ...
  beforeEach(() => {
    ...
    handler = new MarkInterviewNoShowHandler(
      interviewRepository,
  ...
  it('rejects marking no-show when the application is already terminal', async () => {
    ...
    await expect(
      handler.execute(
        new MarkInterviewNoShowCommand('recruiter-1', 'interview-1'),
      ),
    ).rejects.toThrow(InterviewApplicationNotInterviewableException);
    expect(jobLookupPort.findById).not.toHaveBeenCalled();
  });

  it('allows marking no-show when the application is still in progress', async () => {
    interviewRepository.findById.mockResolvedValue(makeInterview());
    applicationLookupPort.findById.mockResolvedValue({
      id: 'app-1',
      userId: 'candidate-1',
