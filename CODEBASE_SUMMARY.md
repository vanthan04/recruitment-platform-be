...
[SendJobAlertDigestsHandler](src/modules/job-alert/application/commands/send-job-alert-digests.command.ts) chạy mỗi ngày 7h sáng UTC: với mỗi saved search, tìm job mới đăng trong 24h qua khớp điều kiện → gửi mail digest qua `IMailPort`. **Không còn là `@Cron` trong process** — trigger giờ là AWS EventBridge Scheduler gọi [job-alert-digest.handler.ts](src/handlers/job-alert-digest.handler.ts) (Lambda riêng, boot application context không HTTP rồi dispatch `SendJobAlertDigestsCommand` qua `CommandBus`).
...
[CloseExpiredJobsHandler](src/modules/job/application/commands/close-expired-jobs.command.ts) chạy mỗi giờ, tự động `CLOSED` các job `OPEN` đã qua `expiresAt`. **Không còn `@nestjs/schedule`** — trigger là AWS EventBridge Scheduler (`rate(1 hour)`) gọi [close-expired-jobs.handler.ts](src/handlers/close-expired-jobs.handler.ts) (Lambda riêng, dispatch `CloseExpiredJobsCommand` qua `CommandBus`), xem mục "Vận hành" bên dưới.
...
## 4a. Vận hành: deploy AWS Lambda, logging, rate limit
...
- **Deploy target: AWS Lambda, không phải server chạy liên tục.**
  - [src/lambda.ts](src/lambda.ts): entry point cho API Gateway → Lambda, bọc app NestJS bằng `@codegenie/serverless-express`; `cachedHandler` giữ ở module scope để warm invocation tái dùng cùng 1 Nest DI container thay vì bootstrap lại.
  - [src/bootstrap.ts](src/bootstrap.ts): setup dùng chung (`createHttpApp`/`createAppContext`) cho cả `main.ts` (local server) và `lambda.ts`/`handlers/*` — tránh lệch cấu hình giữa 2 môi trường.
  - Cron cũ (`@nestjs/schedule`, `@Cron(...)`) đã **bỏ hoàn toàn** khỏi dependency — thay bằng AWS EventBridge Scheduler gọi 2 Lambda riêng trong [src/handlers/](src/handlers/): [close-expired-jobs.handler.ts](src/handlers/close-expired-jobs.handler.ts) (`rate(1 hour)`) và [job-alert-digest.handler.ts](src/handlers/job-alert-digest.handler.ts) (`cron(0 7 * * ? *)`, UTC). Mỗi handler chỉ boot `createAppContext()` (không HTTP) rồi `commandBus.execute(...)` — business logic vẫn nằm nguyên trong CQRS handler, không lặp lại ở Lambda.
- **Rate limit qua DynamoDB** ([dynamo-throttler-storage.service.ts](src/common/rate-limit/dynamo-throttler-storage.service.ts)): `@nestjs/throttler` mặc định đếm bằng `Map` trong process — sai trên Lambda vì mỗi invocation có thể là 1 execution environment riêng, không chia sẻ memory. `DynamoThrottlerStorage` đếm bằng `UpdateCommand` atomic (`ADD hits`) theo window-bucket key (`throttlerName#key#windowStart`), TTL tự dọn bucket cũ. **Fail-open**: nếu DynamoDB lỗi/unreachable, cho request qua luôn thay vì trả 500 — rate limit là lớp bảo vệ phụ, không phải business logic cốt lõi.
