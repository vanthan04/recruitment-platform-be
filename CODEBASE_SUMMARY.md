...
- **Exception xử lý tập trung**: [GlobalExceptionFilter](src/common/filters/http-exception.filter.ts) (nay là `@Injectable()`, nhận `PinoLogger` qua DI thay vì `new Logger()`) bắt mọi exception, map `DomainException` con (`EntityNotFoundException`→404, `UnauthorizedDomainException`→403, `DuplicateEntityException`→409, `BusinessRuleViolationException`→400), lỗi validation của Nest, **và `Prisma.PrismaClientKnownRequestError` code `P2002`** (unique constraint) → `409 DUPLICATE_ENTITY` (fix 2026-09-05: trước đây rơi vào nhánh `Error` chung, trả `500` thô cho các race check-then-insert như double-apply hay double-create-company). Log kèm `requestId` (sinh bởi `pino-http`, xem mục "Vận hành" bên dưới) và cũng trả `requestId` đó trong `metadata` của response lỗi — dùng để đối chiếu log khi debug.

## 3. Domain model (Prisma schema — [prisma/schema.prisma](prisma/schema.prisma))

| Model | Ghi chú |
|---|---|
| `User` | email/`password` (nullable — user đăng nhập qua OAuth không có password), `googleId`/`facebookId` (nullable, unique — liên kết tài khoản OAuth), `status` (PENDING/ACTIVE/BLOCKED), `roleId` FK tới `Role` (nguồn sự thật duy nhất cho role — cột enum `role` + trigger đồng bộ cũ đã bị xoá ở Phase 5, xem mục 8) |
...
| `OauthLoginCode` | mã dùng-một-lần cho luồng OAuth Google/Facebook (implemented — xem mục `auth` bên dưới); lưu `codeHash` + `userId`, không lưu access/refresh token dạng plaintext |
...
| `Company` | **không còn field `industry`** (xoá ở Phase 4 — nền tảng cam kết IT-only); `companyType` (PRODUCT/OUTSOURCING/STARTUP/CONSULTING) đã expose qua API (`CreateCompanyDto`/`UpdateCompanyDto`/response); `ownerId` có partial unique index (`WHERE deletedAt IS NULL`) — 1 company active/recruiter được enforce ở DB |
...
| `ApplicationStatusHistory` | mới ở Phase 2 — ghi lại mỗi lần đổi status (`fromStatus`, `toStatus`, `note`, `changedById`); đọc lại qua `GET /job-applications/:id/history` |
...
| `CvAnalysis` | Mới (2026-09-12), thuộc module `ai` (không phải `cv` — xem lý do ở mục module `ai` bên dưới) — 1-1 với `Cv` (`cvId` unique). `status` (PENDING/COMPLETED/FAILED), `summary`, `skills: string[]` (chuẩn hoá lowercase), `experienceYears`, `education: string[]`, `extractedText` (không bao giờ log/trả qua API), `model`, `analyzedAt`, `failureReason` |
...
Controller: [AuthController](src/modules/auth/presentation/controllers/auth.controller.ts) — `POST /auth/register|login|verify|forgot-password|reset-password|change-password|logout|refresh`, `GET /auth/google|google/callback|facebook|facebook/callback`, `POST /auth/social/exchange`.
- `AuthService` là facade gọi các use-case (Register, Login, VerifyEmail, ForgotPassword, ResetPassword, ChangePassword) + tự quản lý access/refresh token (access 15m, refresh 7d, refresh token hash bằng bcrypt trước khi lưu DB). Đăng ký công khai chỉ nhận `role: CANDIDATE|RECRUITER` (`@IsIn` trên `RegisterRequestDto`) — `ADMIN` không tự chọn được nữa.
- **OAuth Google/Facebook** (implemented — xem lịch sử ở `GOOGLE_FACEBOOK_LOGIN_PLAN.md` cũ, nay đã xoá vì đã code xong): `passport-google-oauth20`/`passport-facebook` strategy, `GoogleAuthGuard`/`FacebookAuthGuard`. Callback **không** trả JWT thẳng qua URL — tạo 1 mã dùng-một-lần (`OauthLoginCode`, hash + TTL 60s, single-use) rồi redirect FE sang `/auth/callback?code=...`; FE gọi `POST /auth/social/exchange` để đổi mã lấy access/refresh token thật. `state` mang `role` chỉ dùng khi tạo user mới — user đã tồn tại giữ nguyên role cũ.
...
- Cron `cleanup-expired-tokens.cron.ts` chạy hằng ngày, xoá `RefreshToken`/`VerificationToken` đã hết hạn/đã dùng — tránh 2 bảng này phình vô hạn.
...
⚠️ **Field `industry` đã bị xoá hoàn toàn ở Phase 4** (schema, DTO, filter) — nền tảng cam kết chỉ phục vụ IT, không mở rộng đa ngành nữa. Xem [docs/industry-expansion.md](docs/industry-expansion.md). `companyType` (PRODUCT/OUTSOURCING/STARTUP/CONSULTING) cũng như `province`/`ward` đã được wire đầy đủ vào `CreateCompanyDto`/`UpdateCompanyDto`/response.
⚠️ **"1 company/recruiter" giờ có backstop ở DB**: `CreateCompanyHandler` vẫn check-then-insert (`findByOwnerId` trước khi `create`), nhưng từ migration `20260905090000_company_owner_active_unique` có thêm 1 partial unique index `companies(ownerId) WHERE deletedAt IS NULL` — 2 request tạo company gần như đồng thời từ cùng 1 recruiter giờ chỉ 1 cái thành công, cái thua bị Postgres từ chối và `GlobalExceptionFilter` map thành `409 DUPLICATE_ENTITY` (xem mục `common/`). Vì là partial index (không phải `@unique` phẳng), recruiter vẫn tạo được company mới sau khi soft-delete company cũ.
...
[JobController](src/modules/job/presentation/controllers/job.controller.ts): `POST/PATCH/DELETE /jobs` (role RECRUITER, chỉ owner), `GET /jobs` (search công khai, filter theo keyword/location/**employmentType/workMode**/salary/categoryId/companyId/level/**skillIds**/sort + pagination — chỉ trả job `status=OPEN` **và** chưa quá `expiresAt`), `GET /jobs/mine` (mới ở Phase 7 verification pass — recruiter xem toàn bộ job của mình bất kể status), `GET /jobs/:id`, `PATCH /jobs/:id/close`, `PATCH /jobs/:id/reopen` (owner only).
⚠️ **`jobType` (enum gộp cả loại hình lẫn `REMOTE`) đã tách thành `employmentType` + `workMode` độc lập ở Phase 3** — 1 job giờ có thể vừa `FULL_TIME` vừa `REMOTE` cùng lúc. `CreateJobDto` nhận thêm `skillIds?: string[]` gắn vào bảng nối `JobSkill`; `GET /jobs` filter theo `skillIds` match kiểu OR (job có ít nhất 1 skill trong danh sách truyền lên). `extraInfo` (JSON tự do) đã bị thay bằng 2 cột typed `workingHours`/`applicationMethod` — đây là 2 key duy nhất FE từng dùng qua `extraInfo`.
⚠️ **`GET /jobs` lọc thêm `expiresAt`** (job hết hạn nhưng cron hàng giờ chưa kịp đóng vẫn bị ẩn khỏi search) — trước đây search chỉ dựa vào `status='OPEN'`, để lọt job hết hạn tối đa ~1 giờ, khiến candidate bấm apply vào job đã hết hạn và nhận lỗi dù job vẫn "hiện ra" ở kết quả tìm kiếm.
...
⚠️ **Pipeline mở rộng ở Phase 2**: `PENDING/ACCEPTED/REJECTED/WITHDRAWN` (4 giá trị) → `APPLIED→SCREENING→SHORTLISTED→INTERVIEW→OFFER→HIRED` (rẽ `REJECTED` ở mọi bước, `WITHDRAWN` riêng) — 8 giá trị. `job-application.entity.ts` dùng `transitionTo()` theo transition map (`application-status.vo.ts`) thay cho các method `accept()/reject()/withdraw()` cũ. Mỗi lần đổi status ghi 1 dòng `ApplicationStatusHistory`, đọc lại qua `GET /job-applications/:id/history` (chủ đơn hoặc recruiter sở hữu job). Conversation chat giờ mở khi đơn `HIRED` (trước đây `ACCEPTED`).
...
### `ai` — [ai.module.ts](src/modules/ai/ai.module.ts)
**Mới (2026-09-12).** 4 tính năng AI-hỗ-trợ recommend/soạn draft — không tự động đổi status application, không tự tạo/sửa job, không xoá CV. [AiController](src/modules/ai/presentation/controllers/ai.controller.ts): `POST /jobs/:jobId/matching-candidates`, `POST /jobs/:jobId/candidates/:candidateId/screening-questions`, `POST /jobs/skill-suggestions`, `POST /jobs/draft` — chi tiết request/response xem [API_GUIDE.md §4.14](API_GUIDE.md#414-ai-recruitment-agent-jobs-module-ai).

- **Kiến trúc theo đúng convention của repo** (domain/application/infrastructure/presentation), KHÔNG có ngoại lệ nào cho riêng module này — Controller → Application Service (auth/RBAC qua guard, ownership qua `ensureOwner` giống mọi module khác) → Agent/Service → Tool (chỉ gọi qua Query/CommandBus hoặc port có sẵn, không bao giờ import Prisma trực tiếp) → domain service hiện có → Prisma.
- **2 loại "AI-driven" khác nhau, cố tình không đồng nhất thành 1 pattern**: `RecruitmentAgent` (matching) và `ScreeningAgent` (Q&A) là agent thật — LangGraph `StateGraph` 2 node (`agent` gọi model có bind tool ⇄ `tools` chạy `ToolNode`) vì có nhiều tool để agent tự quyết định gọi cái nào. `SkillSuggestionService` và `JobDraftService` chỉ là 1 lệnh gọi model duy nhất ép `tool_choice` — không có gì để "quyết định" gọi thêm ngoài 1 lần tra cứu taxonomy cố định (skill suggestion) hoặc không tra cứu gì cả (job draft), nên không cần vòng lặp LangGraph.
- **`CvAnalysis` thuộc module `ai`, không thuộc `cv`** — quyết định có chủ đích để tránh vòng phụ thuộc module: `ai` import `cv` một chiều (lấy `ICvStoragePort` để tải file CV phân tích), nếu `CvAnalysis` thuộc `cv` thì `cv` sẽ phải import ngược lại `ai` để lưu kết quả phân tích, tạo vòng lặp.
- **Pipeline phân tích CV** (bất đồng bộ, không chặn response upload): `CreateCvHandler` (module `cv`) phát event `cv.uploaded` (1 dòng thêm duy nhất, dùng `EventEmitter2` sẵn có) → [CvEventsListener](src/modules/ai/infrastructure/listeners/cv-events.listener.ts) → `CvAnalysisService.analyzeCv()` — tải file qua `ICvStoragePort.downloadBuffer` (method mới thêm), extract text (`pdf-parse` cho PDF, `mammoth` cho DOCX — **.doc cũ không hỗ trợ**), gọi AI 1 lần ép `tool_choice` để lấy structured output, lưu `CvAnalysis`. [AnalyzePendingCvsCron](src/modules/ai/application/jobs/analyze-pending-cvs.cron.ts) (`EVERY_10_MINUTES`) là lưới an toàn cho CV bị lỡ event hoặc app restart giữa chừng.
- **Lọc xác định (deterministic) trước, AI xếp hạng sau** — `search_candidates`/`matching-candidates` chỉ bao giờ tìm trong các CV đã `CvAnalysisStatus.COMPLETED`; không có nghĩa CV mới upload xuất hiện ngay trong kết quả tìm kiếm.
- **Chống bịa dữ liệu**: schema output của AI luôn validate bằng `zod` (`@langchain/core/tools`' `tool()`), và với các trường hợp có "tập giá trị thật" (candidateId trong matching, skill id trong skill-suggestion) — AI chỉ được chọn trong tập đã thực sự trả về từ tool/DB, ID nào không khớp bị lọc bỏ âm thầm (log warning), không throw lỗi cho cả response.
- **Multi-provider theo từng tính năng**: mỗi trong 5 khả năng AI (matching/cv-analysis/screening/skill-suggestion/job-draft) chọn provider (Anthropic/OpenAI/Gemini) + model độc lập qua env riêng — [chat-model.provider.ts](src/modules/ai/infrastructure/providers/chat-model.provider.ts)`buildChatModel()`.
- ⚠️ **Bug đã fix (2026-09-12, phát hiện qua chạy e2e thật)**: `ChatAnthropic`/`ChatOpenAI`/`ChatGoogleGenerativeAI` validate API key ngay trong constructor và throw đồng bộ nếu thiếu — vì các model này được khởi tạo eager lúc DI instantiate (nằm trong `AiModule` đăng ký global), thiếu `ANTHROPIC_API_KEY` khiến **toàn bộ app crash lúc boot**, không riêng gì endpoint AI. Test unit (dùng fake model) không phát hiện được, chỉ e2e (boot app thật) mới lộ ra. Fix: `buildChatModel()` bắt lỗi construction, trả về 1 `BaseChatModel` giả chỉ throw lúc thực sự bị gọi (`bindTools().invoke()`) — đúng lại nguyên tắc "app luôn boot được, chỉ lỗi lúc gọi endpoint" áp dụng cho mọi biến AI khác.
...
- **Structured logging** (`nestjs-pino` + `pino-http`, cấu hình ở [logger.config.ts](src/common/config/logger.config.ts)): mỗi request có 1 `requestId` (lấy từ header `x-request-id` nếu FE/gateway gửi sẵn, không thì tự sinh UUID) — cùng 1 id nối access log, mọi app log phát sinh lúc xử lý request, và error log ở `GlobalExceptionFilter`. Tự động redact `password`/`refreshToken`/header `authorization`/`cookie`... trước khi log. Dev: pretty-print màu qua `pino-pretty`; production: JSON thuần. Route `GET /api/v1/healthcheck` bị loại khỏi access log tự động (tránh spam log từ health check định kỳ).
  - `AppController` ([app.controller.ts](src/app.controller.ts), route `healthcheck`) đã được khai báo trong `controllers: []` của `AppModule` ([app.module.ts](src/app.module.ts)) — `/api/v1/healthcheck` tồn tại và được loại khỏi access log như mô tả ở trên. (Một bản rà soát trước đây từng ghi nhận thiếu wiring này — đã fix, không còn đúng nữa.)
...
- **Cập nhật số liệu (2026-09-12, sau khi thêm module `ai`)**: `npm test` hiện tại **105 test suite / 460 test, pass 100%**; `npm run test:e2e` **4 suite / 32 test, pass 100%** (chạy trên local docker-compose Postgres, cần `docker compose up -d` trước).
...
- Test hiện có tập trung ở domain entity (business rule, throw exception) và command/query handler (mock repository) — xem số liệu cập nhật ở mục 6a. Repository/infrastructure (Prisma) không có unit test riêng, chỉ được phủ gián tiếp qua e2e (`test/`).
